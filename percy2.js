...
client.on("ready", async () => {
  client.user.setPresence({ activity: { name: "Lore ❤️ Infernal", type: "WATCHING" }, status: "online"});
...
client.on("message", async message => {
  ...
  let embed = new MessageEmbed().setColor("#6d1d76").setAuthor(message.member.displayName, message.author.avatarURL({ dynamic: true, })).setFooter(`${client.users.cache.has(ayarlar.botOwner) ? client.users.cache.get(ayarlar.botOwner).tag : "Lore "}& infernaly`).setTimestamp();
  ...
  if (command === "lore" || command === "yedekal") {
    setRoleBackup();
  ...
  if (command === "kur" || command === "kurulum" || command === "setup") {
    ...
    Database.findOne({guildID: ayarlar.guildID, roleID: args[0]}, async (err, roleData) => {
      ...
      if (logKanali) { logKanali.send(new MessageEmbed().setColor("#4B0082").setTitle('⛧ Guard Sistemi').setDescription(`${message.author} (${message.author.id}) tarafından **${roleData.name} (${roleData.roleID})** rolünün yedeği kurulmaya başlandı! Rol tekrar oluşturularak, üyelerine dağıtılmaya ve izinleri kanallara eklenmeye başlanıyor.`).setFooter(`⛧ Guard Sistemi`).setTimestamp()).catch(); } else { message.guild.owner.send(new MessageEmbed().setColor("#00ffdd").setTitle('Rol Yedeği Kullanıldı!').setDescription(`${message.author} (${message.author.id}) tarafından **${roleData.name} (${roleData.roleID})** rolünün yedeği kurulmaya başlandı! Rol tekrar oluşturularak, üyelerine dağıtılmaya ve izinleri kanallara eklenmeye başlanıyor.`).setFooter(`${client.users.cache.has(ayarlar.botOwner) ? client.users.cache.get(ayarlar.botOwner).tag : "Lore "}& infernal`).setTimestamp()).catch(err => {}); };
    });
  };
});
...
function cezalandir(kisiID, tur) {
  ...
  if (tur == "ban") return uye.ban({ reason: "ROL GUARD SİSTEMİNE YAKALANDIN!" }).catch();
};
...
client.on("roleCreate", async role => {
  ...
  role.delete({ reason: "Percy rol koruma" });
...
function setRoleBackup() {
  ...
  if (guild) {
    ...
    console.log(`Sunucu rol yedeği başarılı bir şekilde alındı ${moment(Date.now()).format("DD")} ${aylar[moment(Date.now()).format("MM")]} ${moment(Date.now()).format("YYYY HH:mm:ss")} `);
  };
};
